...
async function buildQueriesForGroup(country, group) {
  const prompt = KISHOR_CONTEXT + "\n\n" +
    ...
    "Generate 10 search queries to find " + group.category + " brands in " + country + ".\n" +
    "Focus on brands that source from Asia/India.\n" +
    "Return ONLY raw JSON, no markdown:\n" +
    '{"country":"' + country + '","category":"' + group.category + '","group":"' + group.group + '","queries":[{"id":1,"query":"example query","intent":"why"}]}';
...
async function runAgent1(country) {
  console.log("============================================");
  console.log("AGENT 1 — Query Builder | Country: " + country);
  console.log("============================================");
  ...
  for (const group of PRODUCT_GROUPS) {
    ...
    try {
      ...
      totalQueries += result.queries ? result.queries.length : 0;
      console.log("  Generated " + (result.queries ? result.queries.length : 0) + " queries");
      result.queries && result.queries.forEach(q => console.log("  -> " + q.query));
      await new Promise(r => setTimeout(r, 4000));
    } catch(err) {
      console.error("  Error: " + err.message);
      allGroups.push({ country, category: group.category, group: group.group, queries: [] });
      await new Promise(r => setTimeout(r, 5000));
    }
  }

  const output = { country, total_groups: 16, total_queries: totalQueries, generated_at: new Date().toISOString(), groups: allGroups };
  const path = "./output/queries_" + country.toLowerCase().replace(/\s+/g, "_") + "_" + Date.now() + ".json";
  fs.writeFileSync(path, JSON.stringify(output, null, 2));

  console.log("\nAgent 1 Complete! Queries: " + totalQueries + " | Saved: " + path);
  return output;
}
